...
const Chart = ({ data, chartParams, selectedOffice }) => {
	...
	const createHierarchy = (data, root = "candidate", children = ["contributorType"], value = "aggregate") => {
		let nested = d3.nest()
			.key(d => d[root])
		...
		let h = d3.hierarchy(candidateList)
			.sum(d => d.value)
	...
	useEffect(() => {
		...
		rect.on("mouseover", d => {
			...
			tooltip.style("opacity", .9)
				.html(`${d.parent.data.name}<hr />
					${d.data.name}: $${Math.round(d.data.value).toLocaleString()}<br />
					Total: $${Math.round(d.parent.value).toLocaleString()}`
				)
				.style("left", (d3.event.pageX < halfpage ? `${d3.event.pageX + 10}px` : `${d3.event.pageX - 125}px`))
		...
		leaf.each((d, i, arr) => {
			...
			const text = d3.select( current ).append('text')
				.attr("opacity", .9)
				.selectAll("tspan")
				.data(d => [ d.parent.data.name.split(",")[0], `$${Math.round(d.value).toLocaleString()}` ])
				.join("tspan")
	...
	return (
		<>
			...
			<h4>Data from State of Hawaii Campaign Spending Commission (as of 6/30/20)</h4>
		</>
	);
};
